Документация модуля клиента
===============================================================================

Приложение клиент для обмена сообщениями. Отправка осуществляется оплайн клиентам
и шифруются спомощью алгоритма RSA.

Подерживаеммые аргументы:
...
* ``python -n client1 -p 123``

Запуск приложения с пользователем client1 и паролем 123*

* ``python client.py ip_address some_port -n client1 -p 123``

Запуск приложения с пользователем client1 и паролем 123 и указанием подключаться к серверу по адресу ip_address:port*
